...
for (let i = 0; i < femaleCharacters.length; i++) {
  ...
  for (let j = 0; j < characters.length; j++) {
    if (characters[j] === femaleCharacterName) {
...
//OPPURE: (ma non lo abbiamo fatto ancora)
// const filteredCharacters = starWarsCharacters.filter(character => {
//   return !femaleCharacters.some(
//     femaleCharacter => femaleCharacter.name === character.name
//   );
// });
...
console.log(`il personaggio selezionato è ${randomCharacter.name}. è alto ${randomCharacter.height} (non so di quale unità di misura xD) e pesa ${randomCharacter.mass} (forse)kg.
i suoi occhi sono di colore ${randomCharacter.eye_color} e i suoi capelli sono ${randomCharacter.hair_color}.
è nato nel ${randomCharacter.birth_year} ed è ${randomCharacter.gender}.`);
